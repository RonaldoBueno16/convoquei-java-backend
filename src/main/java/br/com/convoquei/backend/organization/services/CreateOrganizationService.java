...
public class CreateOrganizationService {
    ...
    public void create(CreateOrganizationRequest request) {
        ...
        User currentUser = currentUserProvider.user()
                .orElseThrow(() -> new DomainBusinessRuleException("É necessário estar logado para criar uma organização."));

        Organization organization = new Organization(currentUser, request.name(), slug);
